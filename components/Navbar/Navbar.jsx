    ...
    return (
        <div className='bg-black flex justify-between items-center px-5 py-4 h-24'>
            ...
            <div className={`${navbar ? 'bg-slate-600 absolute top-24 inset-x-0 flex flex-col items-center justify-center w-screen' : 'hidden'}`}>
                {routes.map((route) => (
                    <Link className='w-screen text-center p-4 hover:bg-gray-500 rounded-lg transition duration-600 ease-in-out text-white' 
            ...
            <div 
                id='links'
                className={`${styles.regularNavbar} flex-col md:flex md:flex-row md:gap-5 absolute md:relative left-0 w-full md:w-auto bg-black px-5 py-2 text-1xl text-center`}
            >
                {routes.map((route) => (
                    <Link className='p-2 hover:bg-gray-500 rounded-lg transition duration-600 ease-in-out text-white' 
                        href={route.path} key={route.title}
                    >
                        {route.title}
                    </Link>
