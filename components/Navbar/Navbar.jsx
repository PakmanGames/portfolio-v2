    ...
    return (
        <div className='bg-black flex justify-between items-center px-5 py-4 h-24'>
            ...
            <div className={`${navbar ? 'bg-slate-600 absolute top-24 inset-x-0 flex flex-col items-center justify-center w-screen' : 'hidden'}`}>
                {routes.map((route) => (
                    <Link className={`w-screen p-4 hover:bg-gray-100 ${currentRoute === route.path ? 'bg-gray-500 text-white' : 'text-white'} transition duration-600 ease-in-out text-white text-xl text-center`} 
            ...
            >
                {routes.map((route) => (
                    <Link className={`p-3 rounded-2xl hover:bg-gray-100 hover:text-black ${currentRoute === route.path ? 'bg-gray-400 text-black' : 'text-white'} transition duration-600 ease-in-out text-xl text-center`} 
                        href={route.path} key={route.title}
                    >
                        {route.title}
                    </Link>
